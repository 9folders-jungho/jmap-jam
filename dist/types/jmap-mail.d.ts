...
export type Mailbox = {
    ...
    name: string;
    parentId: ID | null;
    role: "all" | "archive" | "drafts" | "flagged" | "important" | "inbox" | "junk" | "sent" | "subscribed" | "trash" | null;
    sortOrder: number;
    totalEmails: number;
    unreadEmails: number;
    totalThreads: number;
 ...
 */
export type Email = EmailMetadataFields & EmailHeaderFields & EmailBodyPartFields & {
 ...
 */
export type HeaderFieldKey = `header:${string}:as${keyof HeaderParsedForm}:all` | `header:${string}:as${keyof HeaderParsedForm}` | `header:${string}:all` | `header:${string}`;
...
export {};
//# sourceMappingURL=jmap-mail.d.ts.map